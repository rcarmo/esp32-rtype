...
bool rtype_i86_step(rtype_i86_cpu_t *cpu) {
    ...
    case 0x00: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,reg=(mr>>3)&7,rm=mr&7; uint32_t a=0; uint8_t dst=(mod==3)?*reg8(cpu,rm):(a=ea(cpu,mod,rm),rb(cpu,a)); uint8_t res=(uint8_t)(dst+*reg8(cpu,reg)); set_add8(cpu,dst,*reg8(cpu,reg),res); if(mod==3)*reg8(cpu,rm)=res; else wb(cpu,a,res); return true; }
    ...
    case 0x04: { uint8_t imm=fetch8(cpu); uint8_t old=*reg8(cpu,0); *reg8(cpu,0)=(uint8_t)(old+imm); set_add8(cpu,old,imm,*reg8(cpu,0)); return true; }
    ...
    case 0x35: { uint16_t imm=fetch16(cpu); cpu->r[RTYPE_I86_AX]^=imm; set_logic16(cpu,cpu->r[RTYPE_I86_AX]); return true; }
    ...
    case 0x80: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,sub=(mr>>3)&7,rm=mr&7; uint32_t a=0; uint8_t v=(mod==3)?*reg8(cpu,rm):(a=ea(cpu,mod,rm),rb(cpu,a)); uint8_t imm=fetch8(cpu),res=v; bool write=true; if(sub==0){res=(uint8_t)(v+imm);set_add8(cpu,v,imm,res);} else if(sub==1){res=(uint8_t)(v|imm);set_logic8(cpu,res);} else if(sub==4){res=(uint8_t)(v&imm);set_logic8(cpu,res);} else if(sub==5){res=(uint8_t)(v-imm);set_sub8(cpu,v,imm,res);} else if(sub==6){res=(uint8_t)(v^imm);set_logic8(cpu,res);} else if(sub==7){res=(uint8_t)(v-imm);set_sub8(cpu,v,imm,res);write=false;} else {halt_reason(cpu,"unsupported 80/%u at %05x",sub,before);return false;} if(write){if(mod==3)*reg8(cpu,rm)=res; else wb(cpu,a,res);} return true; }
    case 0x81: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,sub=(mr>>3)&7,rm=mr&7; uint32_t a=0; uint16_t v=(mod==3)?cpu->r[rm]:(a=ea(cpu,mod,rm),rw(cpu,a)); uint16_t imm=fetch16(cpu),res=v; bool write=true; if(sub==0){res=(uint16_t)(v+imm);set_add16(cpu,v,imm,res);} else if(sub==1){res=(uint16_t)(v|imm);set_logic16(cpu,res);} else if(sub==4){res=(uint16_t)(v&imm);set_logic16(cpu,res);} else if(sub==5){res=(uint16_t)(v-imm);set_sub16(cpu,v,imm,res);} else if(sub==7){res=(uint16_t)(v-imm);set_sub16(cpu,v,imm,res);write=false;} else {halt_reason(cpu,"unsupported 81/%u at %05x",sub,before);return false;} if(write){if(mod==3)cpu->r[rm]=res; else ww(cpu,a,res);} return true; }
    case 0x83: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,sub=(mr>>3)&7,rm=mr&7; uint32_t a=0; uint16_t v=(mod==3)?cpu->r[rm]:(a=ea(cpu,mod,rm),rw(cpu,a)); uint16_t imm=(uint16_t)(int16_t)(int8_t)fetch8(cpu),res=v; bool write=true; if(sub==0){res=(uint16_t)(v+imm);set_add16(cpu,v,imm,res);} else if(sub==1){res=(uint16_t)(v|imm);set_logic16(cpu,res);} else if(sub==5){res=(uint16_t)(v-imm);set_sub16(cpu,v,imm,res);} else if(sub==7){res=(uint16_t)(v-imm);set_sub16(cpu,v,imm,res);write=false;} else {halt_reason(cpu,"unsupported 83/%u at %05x",sub,before);return false;} if(write){if(mod==3)cpu->r[rm]=res; else ww(cpu,a,res);} return true; }
    case 0x84: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,reg=(mr>>3)&7,rm=mr&7; uint8_t v=(mod==3)?*reg8(cpu,rm):rb(cpu,ea(cpu,mod,rm)); set_logic8(cpu,(uint8_t)(v & *reg8(cpu,reg))); return true; }
    case 0x85: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,reg=(mr>>3)&7,rm=mr&7; uint16_t v=(mod==3)?cpu->r[rm]:rw(cpu,ea(cpu,mod,rm)); set_logic16(cpu,(uint16_t)(v & cpu->r[reg])); return true; }
    case 0x86: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,reg=(mr>>3)&7,rm=mr&7; if(mod==3){uint8_t tmp=*reg8(cpu,rm); *reg8(cpu,rm)=*reg8(cpu,reg); *reg8(cpu,reg)=tmp;} else {uint32_t a=ea(cpu,mod,rm); uint8_t tmp=rb(cpu,a); wb(cpu,a,*reg8(cpu,reg)); *reg8(cpu,reg)=tmp;} return true; }
    case 0x87: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,reg=(mr>>3)&7,rm=mr&7; if(mod==3){uint16_t tmp=cpu->r[rm]; cpu->r[rm]=cpu->r[reg]; cpu->r[reg]=tmp;} else {uint32_t a=ea(cpu,mod,rm); uint16_t tmp=rw(cpu,a); ww(cpu,a,cpu->r[reg]); cpu->r[reg]=tmp;} return true; }
    ...
    case 0x8f: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,sub=(mr>>3)&7,rm=mr&7; if(sub){halt_reason(cpu,"unsupported 8F/%u at %05x",sub,before);return false;} uint16_t v=pop(cpu); if(mod==3)cpu->r[rm]=v; else ww(cpu,ea(cpu,mod,rm),v); return true; }
    ...
    case 0xa8: { uint8_t imm=fetch8(cpu); set_logic8(cpu,(uint8_t)(*reg8(cpu,0)&imm)); return true; }
    ...
    case 0xc6: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,sub=(mr>>3)&7,rm=mr&7; if(sub){halt_reason(cpu,"unsupported C6/%u at %05x",sub,before);return false;} uint32_t a=0; if(mod!=3)a=ea(cpu,mod,rm); uint8_t imm=fetch8(cpu); if(mod==3)*reg8(cpu,rm)=imm; else wb(cpu,a,imm); return true; }
    case 0xc7: { uint8_t mr=fetch8(cpu); unsigned mod=mr>>6,sub=(mr>>3)&7,rm=mr&7; if(sub){halt_reason(cpu,"unsupported C7/%u at %05x",sub,before);return false;} uint32_t a=0; if(mod!=3)a=ea(cpu,mod,rm); uint16_t imm=fetch16(cpu); if(mod==3)cpu->r[rm]=imm; else ww(cpu,a,imm); return true; }
