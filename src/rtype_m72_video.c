...
void rtype_m72_video_render_cyd_strip(const rtype_m72_video_t *video, uint16_t *dst,
    ...
    for (unsigned row = 0; row < rows; row++) {
        ...
        for (unsigned x = 0; x < RTYPE_BLIT_CYD_LOGICAL_W; x++) {
            ...
            if (sp_hit) px = sp;
            out[x] = rtype_blit_rgb565_identity(px);
        }
    }
}
...
static void render_cyd_columns_impl(const rtype_m72_video_t *video, uint16_t *dst,
    ...
    if (!include_sprites) {
        // Full-width snapshots remove chunk/mixed-frame trails. Use a coarse
        // 4x4 background sampler on the tiny CYD panel to improve display FPS;
    ...
    for (unsigned py = 0; py < RTYPE_BLIT_CYD_PHYS_H; py++) {
        ...
        if (!all_active) {
            for (unsigned c = 0; c < cols; c++) {
                ...
                out[c] = rtype_blit_rgb565_identity(px);
            }
            continue;
        }
